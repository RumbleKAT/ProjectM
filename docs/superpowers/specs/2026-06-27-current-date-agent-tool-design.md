...
Reduce hallucinated answers to questions about today's date or weekday by giving the agent a deterministic `get-current-date` tool. The tool calculates the answer at call time instead of relying on the model's training data or prompt memory.
...
The built-in plugin is named `get-current-date` and is enabled as a default agent skill. It accepts no model-provided parameters, preventing the model from substituting a guessed date or time zone.
...
The model may translate or reformat this result for the user, but it must preserve the returned date and weekday.
...
Only the validated time-zone identifier is retained. The current date itself is never stored because a long-lived agent session can cross midnight; the tool calculates it fresh on every call.

## Agent Guidance

The workspace agent definition includes a short temporal accuracy rule whenever `get-current-date` is available:

1. For questions asking for today's/current date or weekday, call `get-current-date` before answering.
2. Never infer the current date from model knowledge or previous messages.
3. Preserve the date and weekday returned by the tool.
...
- `server/utils/agents/aibitat/plugins/current-date.js` owns time-zone validation, deterministic date formatting, and the AIbitat plugin definition.
...
- Date formatting uses `Intl.DateTimeFormat(...).formatToParts()` rather than locale-dependent string parsing.
...
- A Seoul browser at a UTC date boundary receives the correct next-day date and weekday.
- A negative-offset browser receives the correct previous-day date and weekday.
...
- `get-current-date` is present in the default agent function list and the agent role requires its use for current-date questions.
